...
6. 전문 분야가 감지되면 출처, 검수자, 면책 문구, 금지 표현 컨텍스트를 만든다.
7. 카드별 GPT 이미지 생성용 최종 프롬프트를 만든다.
8. `output/<topic>/prompt-01.md`부터 카드별 프롬프트 파일을 저장한다.
9. 사용자는 프롬프트를 한 장씩 GPT 이미지 생성 기능에 넣고 결과 이미지를 검수한다.
...
전문 분야 컨텍스트는 `domain`, `contentType`, `sources`, `review`, `disclaimer`, `forbiddenClaims`를 사용한다. 도메인이 지정되지 않아도 주제 키워드로 보수적으로 감지한다.
...
- 복잡한 전문 주제는 로컬 규칙 기반 문구가 부족할 수 있다. 이 경우 JSON에서 대상, 목적, 장수, 디자인 프리셋을 함께 지정하는 편이 안정적이다.
- 이미지 생성 결과가 카드 간 스타일이 달라질 수 있다. 같은 주제의 카드들은 같은 `prompts.md` 세트에서 순서대로 생성하고, 마음에 들지 않는 카드는 `--card N`으로 재생성 프롬프트만 뽑는다.
...
- 특정 카드만 프롬프트로 다시 출력할 수 있는지 확인한다.
- 이전 PNG/HTML 산출물이 같은 출력 폴더에 남아 있으면 정리되는지 확인한다.
